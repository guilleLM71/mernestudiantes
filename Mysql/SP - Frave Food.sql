...
BEGIN
	INSERT INTO person (nombres, apellidos)
	VALUE (nombres, apellidos);
	
	INSERT INTO users (username, email, passwordd, idpersona, idrol, token) 
	VALUE (nombres, email, pass, LAST_INSERT_ID(), rol, nToken);
END//
...
CREATE PROCEDURE SP_LOGIN(
	IN email VARCHAR(100))
BEGIN
	SELECT p.idpersona, p.nombres, p.apellidos, u.email, u.passwordd, u.idrol, u.token 
	FROM person p
...
DELIMITER //
CREATE PROCEDURE SP_RENEWTOKENLOGIN(IN idpersona INT )
BEGIN
	SELECT p.idpersona, p.nombres, p.apellidos,u.email, u.idrol, u.token
	 FROM person p
	INNER JOIN usuarios u ON p.idpersona = u.idpersona
